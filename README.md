...
```sh
docker tag sp-inference-service:1.0.0 <REGISTRY>/<REPO>/sp-inference-service:1.0.0
```
...
```sh
docker push <REGISTRY>/<REPO>/sp-inference-service:1.0.0
```
...
2. Pass the docker image URI (e.g `<REGISTRY>/<REPO>/sp-inference-service:1.0.0`) when creating/editing the inference service
